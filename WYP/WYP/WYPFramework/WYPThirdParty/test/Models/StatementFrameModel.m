...
- (void)setStatement:(StatementModel *)statement {
    ...
    _nameF = CGRectMake(CGRectGetMaxX(_headImageF) + space, CGRectGetMinY(_headImageF), nameSize.width, nameSize.height);
    
    // 时间
    _timeF = CGRectMake(CGRectGetMaxX(_headImageF) + space, CGRectGetMaxY(_headImageF) - 10, 80, 10);
    
    // 消息
    CGSize messageSize = [self sizeWithText:statement.message font:messageFont maxSize:CGSizeMake(kScreen_width - 2 * space, 1000)];
    _messageHeight = messageSize.height;
    if (messageSize.height > 100) {
        // 判断是否展开
        if (_isShowAllMessage) {
            _messageF = CGRectMake(space, CGRectGetMaxY(_headImageF) + space, messageSize.width, messageSize.height);
        } else {
            _messageF = CGRectMake(space, CGRectGetMaxY(_headImageF) + space, messageSize.width, 100);
        }
    } else {
        _messageF = CGRectMake(space, CGRectGetMaxY(_headImageF) + space, messageSize.width, messageSize.height);
    }
    ...
    CGFloat imageX = 0.0;
    CGFloat imageY = 0.0;
    CGFloat imageW = (kScreen_width - 2 * space - 2 * imageSpace) / 3;
    ...
    for (NSInteger i = 0; i < imageCount; i ++) {
        if (imageCount == 0) {
            ...
            imageH = 0;
            if (messageSize.height > 100) {
                _imageArrayF = CGRectMake(space, CGRectGetMaxY(_messageF) + space + 30, imageW, imageH);
            } else {
                _imageArrayF = CGRectMake(space, CGRectGetMaxY(_messageF) + space, imageW, imageH);
            }
...
//            _imageArrayF = CGRectMake(space, CGRectGetMaxY(_messageF) + space, 2 * imageW + imageSpace, imageY + imageH);
            ...
            if (messageSize.height > 100) {
            _imageArrayF = CGRectMake(space, CGRectGetMaxY(_messageF) + space + 30, kScreen_width - 2 * space, imageY + imageH);
            } else {
            _imageArrayF = CGRectMake(space, CGRectGetMaxY(_messageF) + space, kScreen_width - 2 * space, imageY + imageH);
            }
        }
    ...
    if (imageCount == 0) {
        // 没有图片
        _shareF = CGRectMake(kScreen_width - space - 25, CGRectGetMaxY(_messageF) + space, 25, 35);
    } else {
        // 有图片
        _shareF = CGRectMake(kScreen_width - space - 25, CGRectGetMaxY(_imageArrayF) + space, 25, 35);
    }
    ...
    for (StarAndCommentModel * star in statement.starArray) {
        // 计算昵称尺寸
        CGSize starSize = [self sizeWithText:[NSString stringWithFormat:@"%@   ", star.nickName] font:nameFont maxSize:CGSizeMake(kScreen_width - 2 * space, 30)];
    ...
    _starArrayF = CGRectMake(space, CGRectGetMaxY(_shareF) + space, kScreen_width - 2 * space, starY + starH);
...
//    NSInteger commentRow = 0;
    ...
    for (StarAndCommentModel * comment in statement.commentArray) {
        ...
        nickNameX = 0.0;
        nickNameY = CGRectGetMaxY(lastCommentTextRect) + imageSpace;
        ...
        commentW = kScreen_width - 2 * space;
    ...
    if (statement.starArray.count == 0) {
        // 没有人点赞
        _commentArrayF = CGRectMake(space, CGRectGetMaxY(_shareF) + space, kScreen_width - 2 * space, commentY + commentH);
    } else {
        // 有人点赞
        _commentArrayF = CGRectMake(space, CGRectGetMaxY(_starArrayF) + space, kScreen_width - 2 * space, commentY + commentH);
    }
