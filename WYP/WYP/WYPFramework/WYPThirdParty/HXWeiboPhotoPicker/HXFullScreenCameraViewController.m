...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didStartRecordingToOutputFileAtURL:(NSURL *)fileURL fromConnections:(NSArray *)connections { 
    // 开始录制
    self.videoTime = 0;
    self.timeLb.text = [NSString stringWithFormat:@"15秒"];
...
- (void)recordingClick:(NSTimer *)timer {
    self.timeLb.text = [NSString stringWithFormat:@"%ld秒",15 - self.videoTime];
    self.videoTime++;
    self.playView.progress = (CGFloat)self.videoTime / 15.f;
    if (self.videoTime == 15) {
        [timer invalidate];
        self.timer = nil;
    }
}
