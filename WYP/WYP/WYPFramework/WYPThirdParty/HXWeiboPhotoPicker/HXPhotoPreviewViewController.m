...
- (void)setup
{
    ...
    UICollectionView *collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(-10, 0, width + 20, height - 64) collectionViewLayout:flowLayout];
    ...
    if (self.selectedComplete) {
        ...
        collectionView.frame =  CGRectMake(-10, 0, width + 20, height);
...
//        navItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"取消" style:UIBarButtonItemStylePlain target:self action:@selector(dismissClick)];
        ...
        topView.backgroundColor = [UIColor colorWithRed:220/255.0 green:58/255.0 blue:32/255.0 alpha:1];
        [self.view addSubview:topView];
        
        UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
        [btn addTarget:self action:@selector(dismissClick) forControlEvents:UIControlEventTouchUpInside];
        btn.frame = CGRectMake(0, 20, 80, 44);
        [btn setTitle:@"取消" forState:UIControlStateNormal];
        [topView addSubview:btn];
        
        [topView addSubview:self.titleLb];
        
        self.titleLb.frame = CGRectMake((width - 100)/2, 30, 100, 20);
