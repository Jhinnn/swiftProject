...
/// 获取原始顺序的所有联系人的闭包
typealias AddressBookArrayClosure = (_ addressBookArray: [PersonModel])->()
/// 获取按A~Z顺序排列的所有联系人的闭包
typealias AddressBookDictClosure = (_ addressBookDict: [String:[PersonModel]], _ nameKeys: [String])->()

class GetAddressBook: NSObject {
    ...
    public class func requestAddressBookAuthorization() {
        ...
        AddressBookHandle().requestAuthorizationWithSuccessClosure {
            GetAddressBook.getOrderAddressBook(addressBookInfo: { (addressBookDict, nameKeys) in
...
//    }
    ...
    public class func getOriginalAddressBook(addressBookArray success: @escaping AddressBookArrayClosure, authorizationFailure failure: @escaping AuthorizationFailure) {
        ...
        queue.async {
            ...
            AddressBookHandle().getAddressBookDataSource(personModel: { (model) in
    ...
    public class func getOrderAddressBook(addressBookInfo success: @escaping AddressBookDictClosure, authorizationFailure failure: @escaping AuthorizationFailure) {
        ...
        queue.async {
            ...
            var addressBookDict = [String:[PersonModel]]()
            AddressBookHandle().getAddressBookDataSource(personModel: { (model) in
