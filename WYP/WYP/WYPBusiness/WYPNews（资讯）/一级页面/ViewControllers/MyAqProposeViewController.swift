...
class MyAqProposeViewController: BaseViewController {
    ...
    func loadNetData(requestType: RequestType) {
        ...
        NetRequest.myNewTopicListNetRequest(page: "\(pageNumber)", token: AppInfo.shared.user?.token ?? "",uid: AppInfo.shared.user?.userId ?? "") { (success, info, dataArr,gambitCount) in
...
//                self.tableView.mj_header.endRefreshing()
    ...
    lazy var tableView: UITableView = {
        ...
        newsTableView.register(UINib.init(nibName: "QaProposeTableViewCell", bundle: nil), forCellReuseIdentifier: "onePicCell")
...
extension MyAqProposeViewController: UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "onePicCell", for: indexPath) as! QaProposeTableViewCell
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        label.text = "    我的提问(\(self.gamitCount ?? "0"))"
